`timescale 1ns/10ps
module singleRegister_Negative(reset, enable, in, out, clk);
	...
	generate
		for (i = 0; i < 64; i++) begin: loop
			...
			D_FF_Negative flipflop(out[i], tempAll[i], reset, clk);
		end
	endgenerate
...
module singleRegister_Negative_testbench();
	...
	singleRegister_Negative dut(.reset, .enable, .in, .out, .clk);
